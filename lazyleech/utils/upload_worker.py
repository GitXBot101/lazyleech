...
async def _upload_file(client, message, reply, filename, filepath, force_document):
    ...
    upload_wait = await reply.reply_text(f'📂 <b>Starting Upload of :</b> {html.escape(filename)} Will Start in {PROGRESS_UPDATE_DELAY}s')
    ...
    try:
        with tempfile.TemporaryDirectory(dir=str(user_id)) as tempdir:
            ...
            if split_task and not split_task.done():
                await upload_wait.edit_text(f'<b>📂 Splitting :</b> {html.escape(filename)}...')
